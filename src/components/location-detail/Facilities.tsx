...
export default function Faciltites(props){
    ...
    return(
        <>
        { props.c_facilitiesService!== undefined?
<div className="w-full inline-block relative mt-0 lg:mt-14 pb-10  bg-faci-bg  bg-repeat ">
    <div className="container-custom mt-10 mx-auto">
        ...
        <div className=" flex  flex-wrap justify-center">
            {props.c_facilitiesService.map((services,i)=>{
            ...
            <div className=" w-full p-8 lg:px-2 xl:px-2 md:w-1/2 lg:w-1/4">
                <div className=" bg-white rounded shadow-lg w-full pt-12 pb-8 relative">
                    {services.photo.map((photo,i)=>{return(<>
                        <div className=" absolute left-1/2  shadow-lg -translate-x-1/2 -top-9 bg-white rounded-full p-4 w-[65px] h-[65px]"> <img src={photo.url} className="ml-0.5" width="26" alt=""/> </div>
                    </>)})}
                    
                    <div className="w-full text-center py-1  text-blue-primary font-optimabold  text-lg  inline-block">{services.title}</div>
                </div>
            </div>
                </>
              )
